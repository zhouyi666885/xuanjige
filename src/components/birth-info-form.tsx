...
const regionData: Record<string, Record<string, string[]>> = {
  ...
  '四川省': {
    '成都市': ['锦江区','青羊区','金牛区','武侯区','成华区','龙泉驿区','青白江区','新都区','温江区','双流区','郫都区','新津区','金堂县','大邑县','蒲江县','简阳市','都江堰市','彭州市','邛崃市','崇州市'],
  ...
  '台湾省': {
    ...
    '桃园市': ['中区','东区','南区','西区','北区','西屯区','南屯区','北屯区','太平区','大里区','雾峰区','乌日区','丰原区','后里区','石冈区','东势区','和平区','新社区','潭子区','大雅区','神冈区','大肚区','沙鹿区','龙井区','梧栖区','清水区','大甲区','外埔区','大安区'],
    '台中市': ['中区','东区','南区','西区','北区','西屯区','南屯区','北屯区','太平区','大里区','雾峰区','乌日区','丰原区','后里区','石冈区','东势区','和平区','新社区','潭子区','大雅区','神冈区','大肚区','沙鹿区','龙井区','梧栖区','清水区','大甲区','外埔区','大安区'],
...
export function BirthInfoForm({ value: valueProp, onChange, onConfirm, compact }: BirthInfoFormProps) {
  ...
  const cities = useMemo(() => {
    ...
    return Object.keys(regionData[value.province]);
  }, [value.province]);
  const districts = useMemo(() => {
    if (!value.province || !value.city || !regionData[value.province]?.[value.city]) return [];
    return regionData[value.province][value.city];
  }, [value.province, value.city]);

  const handleChange = useCallback((field: keyof BirthInfo, val: string) => {
  ...
  const selectClass = 'w-full bg-ink border border-gold/20 rounded-lg px-3 py-2.5 text-sm text-foreground focus:border-gold/50 focus:outline-none transition-colors appearance-none cursor-pointer';

  return (
    <div className="space-y-4">
      {/* Gender */}
      <div>
        <label className="text-gold/70 text-xs font-medium mb-1.5 block">性别</label>
          ...
          <button
            ...
            className={`py-2.5 rounded-lg border text-sm font-medium transition-all ${
              value.gender === 'male'
                ...
                : 'bg-ink border-gold/10 text-muted-foreground hover:border-gold/30'
            }`}
          >
            ♂ 男
          </button>
      ...
      {/* Birth Date & Time */}
      <div>
        <label className="text-gold/70 text-xs font-medium mb-1.5 block">出生日期</label>
        <div className="grid grid-cols-4 gap-2">
          <select value={value.year} onChange={(e) => handleChange('year', e.target.value)} className={selectClass}>
            <option value="">年</option>
            {years.map((y) => <option key={y} value={y}>{y}年</option>)}
          </select>
          <select value={value.month} onChange={(e) => handleChange('month', e.target.value)} className={selectClass}>
            <option value="">月</option>
            {months.map((m) => <option key={m} value={m}>{m}月</option>)}
          </select>
          <select value={value.day} onChange={(e) => handleChange('day', e.target.value)} className={selectClass}>
            <option value="">日</option>
            {days.map((d) => <option key={d} value={d}>{d}日</option>)}
          </select>
          <select value={value.hour} onChange={(e) => handleChange('hour', e.target.value)} className={selectClass}>
            <option value="">时</option>
            {hours.map((h) => <option key={h} value={h}>{h}时</option>)}
          </select>
          <select value={value.minute} onChange={(e) => handleChange('minute', e.target.value)} className={selectClass}>
            <option value="">分</option>
            {Array.from({length: 60}, (_, i) => i).map((m) => <option key={m} value={String(m)}>{m}分</option>)}
          </select>
        </div>
      </div>
      ...
      {/* Birth Location */}
      <div>
        <label className="text-gold/70 text-xs font-medium mb-1.5 block">出生地点（用于真太阳时校准）</label>
        <div className="grid grid-cols-3 gap-2">
          <select value={value.province} onChange={(e) => handleChange('province', e.target.value)} className={selectClass}>
            <option value="">省/自治区</option>
            {provinces.map((p) => <option key={p} value={p}>{p}</option>)}
          </select>
          <select value={value.city} onChange={(e) => handleChange('city', e.target.value)} className={selectClass} disabled={!value.province}>
            <option value="">市</option>
            {cities.map((c) => <option key={c} value={c}>{c}</option>)}
          </select>
          <select value={value.district} onChange={(e) => handleChange('district', e.target.value)} className={selectClass} disabled={!value.city}>
