...
- 🔴🔴🔴 强制使用技能工具（每本书录入都必须走这个完整流程，缺一不可，不是可选的）：
  ...
  - 🌐【全网数据源覆盖（固定接入18个网站，每次都跑全套）】：
    ...
    所有源都是固定接入的数据源，不是临时去找。每次搜书系统自动遍历18个源，按书籍类型动态排序优先级（古籍优先古籍源、小说优先小说源），但所有源都会被尝试，绝不跳过任何一个。
  - 🔴🔴🔴【Book Finder 与 Book Crawler 共用同一份全网数据源池（必读）】：
    Book Finder 和 Book Crawler 这两个技能的数据源**完全一致、不分家**：
    • Book Finder 的数据源 = 上面列出的全部18个网站（覆盖中国所有能搜到书的网站）
    • Book Crawler 的数据源 = 上面列出的全部18个网站（与 Book Finder 完全相同）
    • **不存在某个技能只搜古籍网站、另一个技能只搜小说网站的分工**
    • **不存在某个技能只搜中文网站、另一个技能只搜英文网站的分工**
    • 任何一本书，两个技能都会从这18个网站里去搜索和爬取，没有任何"分类优先级"的限制
    • 已取消"古籍优先 / 小说优先"的排序，所有18个源平等遍历——书是什么类型，都跑全套
    • 中国有多少个能搜到书的网站，这18个源就要覆盖多少个；如果遗漏，要持续追加，让两个数据源都达到全网覆盖
