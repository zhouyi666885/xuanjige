...
export async function POST(request: NextRequest) {
  try {
    ...
    const knowledgeResults = await searchKnowledge(searchQuery, 15, 0.15);
    ...
    const knowledgeIronLaw = knowledgeResults.length > 0
      ? '\n\n🔴🔴🔴【知识库铁律——永久生效】🔴🔴🔴\n你的知识库中已有上述检索到的典籍论断。你的回答必须遵循：\n1. ⚠️ 你必须把知识库中所有相关典籍的论断全部列出，一条都不能漏！绝不能只挑一两本书的一两条就下结论！不同典籍对同一问题可能有不同论断，你必须全部呈现出来交叉验证\n2. 先逐条列出知识库中每本典籍的相关论断，每条都要标注来自哪本书\n3. 列出所有论断后，再结合用户命盘特征，对所有论断进行交叉验证\n4. 只有当多本典籍（至少3本以上）的论断指向同一结论时，才能得出"可信度较高"的判断\n5. 如果典籍之间论断有矛盾，必须如实列出矛盾点，说明不同派别的不同观点，不能只选一个\n6. 给出最终判断时，必须说明"本判断综合了《某某》《某某》《某某》中关于某某的论断"\n7. 不引用知识库内容就直接回答的判断，视为无效\n8. 你的所有判断必须且只能来自知识库中真实存在的典籍论断，禁止凭空编造任何书籍名称、论断内容或案例\n9. 知识库中没有的内容就是不知道，必须如实说"知识库中未找到相关典籍论断"，绝不许自行编造'
